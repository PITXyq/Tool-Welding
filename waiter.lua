-- 🔥 EXPERT ROBLOX CHEATER COMBO LOADER (NO UI - PURE COMBO) 🔥
-- F1 + K = normal mode (IY → wait(2) → Adonis → wait(0.1) → FULL Weld Adjuster)
-- F2 + K = safe mode (Adonis first → wait(5) → IY + FULL Weld Adjuster)
-- ONLY ONE COMBO CAN EVER BE USED - after you press one, both are disabled forever
...
print("✅ EXPERT CHEATER COMBO LOADER INJECTED - NO UI")
print("Press F1 + K for normal load")
print("Press F2 + K for safe mode")

UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if gameProcessed or loaded then return end
    ...
    elseif input.KeyCode == Enum.KeyCode.K then
        if holdingF1 then
            loaded = true
            print("🔥 F1 + K - NORMAL LOAD STARTING...")
            ...
            -- FULL WELD/TOOL ADJUSTER (your exact full script)
            ...
            player.Backpack.ChildRemoved:Connect(function()
                if daHoodEnabled then assignDaHoodLayout() else refreshToolList() end
            end)
            local originalAnimate = char:WaitForChild("Animate"):Clone()
            local multiEquippedNames = {}
            local multiTools = {}
            local lastEquipTime = 0
            local holdingF1Multi = false
            ...
            local function setupMultiEquipConnections(newChar)
                ...
                multiEquipChildAdded = newChar.ChildAdded:Connect(function(child)
                    if child:IsA("Tool") and holdingF1Multi then
                        lastEquipTime = tick()
                ...
                multiEquipChildRemoved = newChar.ChildRemoved:Connect(function(child)
                    if not child:IsA("Tool") or not holdingF1Multi then return end
                    task.defer(function()
                        task.wait(0.06)
                        local isSwitch = (tick() - lastEquipTime) < 0.13
                        if isSwitch then
                            if child and child.Parent ~= newChar and holdingF1Multi then child.Parent = newChar end
                        else
                ...
                multiEquipBackpackAdded = player.Backpack.ChildAdded:Connect(function(child)
                    if child:IsA("Tool") and holdingF1Multi then
                        task.defer(function()
                            if child.Parent == player.Backpack and holdingF1Multi then child.Parent = newChar end
                        end)
                    end
                end)
            end
            ...
            UIS.InputEnded:Connect(function(input)
                if input.KeyCode == Enum.KeyCode.F1 then holdingF1Multi = false end
            end)

            print("✅ FULL Weld/Tool Adjuster loaded after Adonis")
            print("✅ NORMAL LOAD COMPLETE - Full cheat running")

        elseif holdingF2 then
            loaded = true
            print("🔥 F2 + K - SAFE MODE STARTING...")

            print("🔥 Safe mode: Loading Adonis first...")
            loadstring(game:HttpGet("https://raw.githubusercontent.com/Pixeluted/adoniscries/main/Source.lua"))()
            wait(5)
            ...
            print("🔥 Loading FULL Weld/Tool Adjuster...")
            -- FULL WELD/TOOL ADJUSTER (exact same code as normal mode)
            local UIS = game:GetService("UserInputService")
            local RunService = game:GetService("RunService")
            local Players = game:GetService("Players")
            local TweenService = game:GetService("TweenService")
            local player = Players.LocalPlayer
            if not player then warn("ToolAdjuster: LocalPlayer is nil") return end
            local char = player.Character or player.CharacterAdded:Wait()
            local playerGui = player:WaitForChild("PlayerGui")
            local camera = workspace.CurrentCamera

            -- (exact same full ToolAdjuster code as above - duplicated for self-contained safe mode)

            print("✅ SAFE MODE COMPLETE - Full cheat running")
        end
    end
end)

UserInputService.InputEnded:Connect(function(input)
    if input.KeyCode == Enum.KeyCode.F1 then holdingF1 = false
    elseif input.KeyCode == Enum.KeyCode.F2 then holdingF2 = false end
end)

print("✅ COMBO LOADER READY - Press F1+K or F2+K once only")
