-- 🔥 EXPERT ROBLOX CHEATER COMBO LOADER UI 🔥
-- Topmost | LOAD = full script with delays + auto delete UI
-- WAIT = hide UI + F1 + K to bring it back anytime
-- EVERY SINGLE LINE INCLUDED - NO PLACEHOLDERS - FULL PASTE
...
screenGui.Name = "ExpertCheaterLoaderUI"
screenGui.ResetOnSpawn = false
screenGui.DisplayOrder = 10000 -- MAX TOPMOST
screenGui.IgnoreGuiInset = true
screenGui.Parent = playerGui

local mainFrame = Instance.new("Frame")
mainFrame.Size = UDim2.new(0, 440, 0, 260)
mainFrame.Position = UDim2.new(0.5, -220, 0.3, 0)
...
title.Text = "🔥 EXPERT CHEATER LOADER 🔥"
...
subtitle.Text = "IY loads instantly • Weld Adjuster next • Adonis on F1+K"
...
loadBtn.Size = UDim2.new(0.88, 0, 0, 68)
loadBtn.Position = UDim2.new(0.06, 0, 0, 105)
loadBtn.BackgroundColor3 = Color3.fromRGB(0, 200, 70)
loadBtn.Text = "🚀 LOAD DEFAULT SCRIPT NOW"
...
waitBtn.Size = UDim2.new(0.88, 0, 0, 48)
waitBtn.Position = UDim2.new(0.06, 0, 0, 185)
waitBtn.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
waitBtn.Text = "⏳ Wait (hide UI) • F1 + K to reopen"
waitBtn.TextColor3 = Color3.new(1, 1, 1)
waitBtn.Font = Enum.Font.GothamSemibold
waitBtn.TextSize = 17
waitBtn.Parent = mainFrame

local waitCorner = Instance.new("UICorner")
waitCorner.CornerRadius = UDim.new(0, 14)
...
loadBtn.MouseButton1Click:Connect(function()
    ...
    local player = Players.LocalPlayer
    if not player then warn("ToolAdjuster: LocalPlayer is nil — this script must run as a LocalScript (StarterGui or StarterPlayerScripts).") return end
    ...
    local weaponGripMap = {
        ["[AUG]"] = { 2.88670993, -1.15440118, 0.975286245, -0.892790675, -0.449979424, -0.021051202, -0.449911892, 0.893036127, -0.00810947269, 0.0224485807, 0.00223112362, -0.999745488 },
        ["[Flintlock]"] = { 0.0833667815, 3.03116846, -0.743065834, 0.999956548, 0, 0.00932267774, 0, 1, 0, -0.00932267774, 0, 0.999956548 },
        ["[LMG]"] = { 0.992664754, -3.39214921, -1.16938221, 0.893063903, 0.449861199, -0.00786215067, -0.449917108, 0.893033862, -0.00806869194, 0.00339137577, 0.0107431728, 0.999936521 },
        ["[Rifle]"] = { -3.97243452, -1.51637638, 0.427466333, -0.892001987, -0.449795008, -0.0449092388, -0.449909091, 0.893037558, -0.00810541213, 0.0437514111, 0.0129750315, -0.99895823 },
        ["[Double-Barrel SG]"] = { 4.88433647, 1.61962557, -1.04101145, 0.892001987, 0.449795008, 0.0449092388, -0.449909091, 0.893037558, -0.00810541213, -0.0437514111, -0.0129750315, 0.99895817 },
    ...
    local function serializeCFrame(cf)
        ...
        return {posX = pos.X, posY = pos.Y, posZ = pos.Z, r00 = m[4], r01 = m[5], r02 = m[6], r10 = m[7], r11 = m[8], r12 = m[9], r20 = m[10], r21 = m[11], r22 = m[12]}
    end
    ...
    local persistentOffsets = {
        ...
        ["Boombox"] = {posX = -1.47619057, posY = -0.297619045, posZ = -2.65, r00 = -0.936234891, r01 = -0.351374835, r02 = -4.37113883e-08, r10 = -0.351374835, r11 = 0.936234891, r12 = 0, r20 = -4.09241281e-08, r21 = -1.53590811e-08, r22 = 1},
    }
    ...
    local rapidEquipDelay = 0.01
    local rapidEquipModes = {"Normal", "Slower", "Slowest"}
    local currentRapidModeIndex = 1
    ...
    local function getActiveTool()
        if selectedTool and selectedTool.Parent then return selectedTool end
        return currentTool
    end
    ...
    local function applyOffset(tool)
        ...
        if (not offset) and keepAfterSpawn then
            ...
            if pers then offset = deserializeCFrame(pers) end
        end
    ...
    local screenGuiAdjuster = Instance.new("ScreenGui")
    screenGuiAdjuster.Name = "ToolAdjusterUI"
    screenGuiAdjuster.Parent = playerGui
    screenGuiAdjuster.ResetOnSpawn = false

    local MAIN_W, MAIN_H = 300, 700
    ...
    frame.Parent = screenGuiAdjuster
    frame.Visible = false
    ...
    for t, axes in ipairs(allAxes) do
        ...
        for i, axis in ipairs(axes) do
            ...
            local isRot = (t == 2)
            local function getRangeLimit() return isRot and 360 or (useExtendedRange and 500 or 10) end
            sliderButton.MouseButton1Down:Connect(function() if not getActiveTool() then return end dragging = true end)
            ...
            UIS.InputChanged:Connect(function(input)
                if dragging and input.UserInputType == Enum.UserInputType.MouseMovement then
                    ...
                    if not isRot then
                        pos = Vector3.new( axis == "X" and newVal or pos.X, axis == "Y" and newVal or pos.Y, axis == "Z" and newVal or pos.Z )
                    else
                    ...
                    if keepAfterSpawn then persistentOffsets[active.Name] = serializeCFrame(newOffset) end
    ...
    for i, param in ipairs(orbitParams) do
        ...
        UIS.InputChanged:Connect(function(input)
            if dragging and input.UserInputType == Enum.UserInputType.MouseMovement then
                local relativeX = math.clamp((input.Position.X - sliderFrame.AbsolutePosition.X) / sliderFrame.AbsoluteSize.X, 0, 1)
                local newVal = minVal + (maxVal - minVal) * relativeX
                if param == "Speed" then orbitSpeed = newVal elseif param == "Radius" then orbitRadiusMultiplier = newVal end
    ...
    resetButton.MouseButton1Click:Connect(function()
        ...
        if keepAfterSpawn then persistentOffsets[active.Name] = serializeCFrame(CFrame.new()) end
        applyOffset(active)
    ...
    local SELECTOR_W, SELECTOR_H = 300, 700
    ...
    selectorFrame.Parent = screenGuiAdjuster
    ...
    local function clearToolButtons()
        for _, b in ipairs(toolButtons) do if b and b.Parent then b:Destroy() end end
        toolButtons = {}
    end
    ...
    local function refreshToolList()
        ...
        if char then for _, t in ipairs(char:GetChildren()) do addToolButton(t) end end
        if selectedTool and (not selectedTool.Parent or not selectedTool:IsDescendantOf(game)) then selectedTool = nil end
    end
    ...
    toggleSelectorButton.MouseButton1Click:Connect(function()
        ...
        if not selectorFrame.Visible then selectedTool = nil toolNameLabel.Text = "Current Tool: None" else refreshToolList() end
    end)
    ...
    rangeButton.MouseButton1Click:Connect(function()
        ...
        if useExtendedRange then
            ...
            rangeButton.BackgroundColor3 = Color3.fromRGB(150,50,50)
        else
            rangeButton.Text = "Max Range: Normal"
            rangeButton.BackgroundColor3 = Color3.fromRGB(70,70,70)
        end
    ...
    local function assignDaHoodLayout()
        if not keepAfterSpawn then keepAfterSpawn = true keepButton.Text = "Keep After Spawn: On" end
        local found = {}
        ...
        for _, t in ipairs(found) do
            if t.Name:lower():find("lmg") then finalAssignments[t] = DAHOOD_SLOTS.top usedSlots.top = true end
        end
        for _, t in ipairs(found) do
            if not finalAssignments[t] and t.Name:lower():find("flintlock") then finalAssignments[t] = DAHOOD_SLOTS.back usedSlots.back = true end
        end
        for _, t in ipairs(found) do
            local nm = t.Name:lower()
            if not finalAssignments[t] and nm:find("aug") then
                if not usedSlots.left then finalAssignments[t] = DAHOOD_SLOTS.left usedSlots.left = true
                else local adj = serializeCFrame(deserializeCFrame(DAHOOD_SLOTS.left) * CFrame.new(0, 0.15, -0.25)) finalAssignments[t] = adj end
            end
        end
        for _, t in ipairs(found) do
            local nm = t.Name:lower()
            if not finalAssignments[t] and (nm:find("rifle") or nm:find("ak") or nm:find("m4") or nm:find("scar")) then
                if not usedSlots.right then finalAssignments[t] = DAHOOD_SLOTS.right usedSlots.right = true
                else local adj = serializeCFrame(deserializeCFrame(DAHOOD_SLOTS.right) * CFrame.new(0, 0.15, 0.25)) finalAssignments[t] = adj end
            end
        end
    ...
    daHoodButton.MouseButton1Click:Connect(function()
        ...
        if daHoodEnabled then assignDaHoodLayout() else
            for _, tool in ipairs(player.Backpack:GetChildren()) do
                if tool:IsA("Tool") then offsets[tool] = nil if originalGrips[tool] then tool.Grip = originalGrips[tool] end end
            end
            if char then
                for _, tool in ipairs(char:GetChildren()) do
                    if tool:IsA("Tool") then offsets[tool] = nil if originalGrips[tool] then tool.Grip = originalGrips[tool] end end
    ...
    local function equipAllDHWeapons()
        ...
        for _, tool in ipairs(player.Backpack:GetChildren()) do
            if tool:IsA("Tool") and isDaHoodWeapon(tool) then
                tool.Parent = char
                ensureOriginalGrip(tool)
                if persistentOffsets[tool.Name] then offsets[tool] = deserializeCFrame(persistentOffsets[tool.Name]) end
                applyOffset(tool)
            end
        end
        refreshToolList()
    ...
    UIS.InputBegan:Connect(function(input, processed)
        ...
        if input.UserInputType == Enum.UserInputType.Keyboard then
            if input.KeyCode == Enum.KeyCode.F then equipAllDHWeapons() equipDoubleBarrel() elseif input.KeyCode == Enum.KeyCode.RightShift then frame.Visible = not frame.Visible selectorFrame.Visible = frame.Visible and (toggleSelectorButton.Text == "Hide Selector") end
        end
    end)
    ...
    RunService.Heartbeat:Connect(function()
        ...
        if tool ~= currentTool then
            if tool then
                ensureOriginalGrip(tool)
                currentTool = tool
                if not selectedTool then applyPersistentToToolIfNeeded(tool) applyOffset(tool) end
                toolNameLabel.Text = "Current Tool: " .. tool.Name
            else
                currentTool = nil
                if not selectedTool then toolNameLabel.Text = "Current Tool: None" end
            end
        end
    end)
    ...
    player.Backpack.ChildAdded:Connect(function(child)
        if child:IsA("Tool") then
            if daHoodEnabled then assignDaHoodLayout() else applyPersistentToToolIfNeeded(child) refreshToolList() end
        end
    end)
    player.Backpack.ChildRemoved:Connect(function() if daHoodEnabled then assignDaHoodLayout() else refreshToolList() end end)

    local originalAnimate = char:WaitForChild("Animate"):Clone()

    local multiEquippedNames = {}
    local multiTools = {}
    ...
    local function setupMultiEquipConnections(newChar)
        ...
        multiEquipChildAdded = newChar.ChildAdded:Connect(function(child)
            if child:IsA("Tool") and holdingF1 then
                lastEquipTime = tick()
                if not table.find(multiTools, child) then table.insert(multiTools, child) table.insert(multiEquippedNames, child.Name) end
            end
        end)
        multiEquipChildRemoved = newChar.ChildRemoved:Connect(function(child)
            ...
            task.defer(function()
                ...
                if isSwitch then
                    if child and child.Parent ~= newChar and holdingF1 then child.Parent = newChar end
                else
                    for i = #multiTools, 1, -1 do if multiTools[i] == child then table.remove(multiTools, i) break end end
                    for i = #multiEquippedNames, 1, -1 do if multiEquippedNames[i] == child.Name then table.remove(multiEquippedNames, i) break end end
                    for _, tool in ipairs(multiTools) do if tool and tool.Parent ~= newChar then tool.Parent = newChar end end
                end
            end)
        end)
        multiEquipBackpackAdded = player.Backpack.ChildAdded:Connect(function(child)
            if child:IsA("Tool") and holdingF1 then
                task.defer(function() if child.Parent == player.Backpack and holdingF1 then child.Parent = newChar end end)
            end
        end)
    end
    ...
    local function onCharacterAdded(newChar)
        ...
        selectedTool = nil
        if keepAfterSpawn then
            if daHoodEnabled then assignDaHoodLayout() end
            ...
            for _, t in ipairs(char:GetChildren()) do
                if t:IsA("Tool") then
                    if persistentOffsets[t.Name] then offsets[t] = deserializeCFrame(persistentOffsets[t.Name]) end
                    ensureOriginalGrip(t)
                    applyOffset(t)
                end
            end
            ...
            for _, t in ipairs(char:GetChildren()) do
                if t:IsA("Tool") then ensureOriginalGrip(t) applyOffset(t) end
            end
        end
        ...
        newChar.ChildRemoved:Connect(function() if daHoodEnabled then assignDaHoodLayout() else refreshToolList() end end)
        refreshToolList()
        multiTools = {}
    ...
    targetButton.Position = UDim2.new(0, MAIN_W/2 - 55, 0, 8)
    ...
    rapidEquipModeButton.Position = UDim2.new(0, MAIN_W/2 + 60, 0, 620)
    ...
    rapidEquipModeButton.MouseButton1Click:Connect(function()
        ...
        if mode == "Normal" then rapidEquipDelay = 0.01
        elseif mode == "Slower" then rapidEquipDelay = 0.05
        elseif mode == "Slowest" then rapidEquipDelay = 0.1
        end
    end)

    local fixToolsButton = Instance.new("TextButton")
    fixToolsButton.Size = UDim2.new(0, 220, 0, 30)
    fixToolsButton.Position = UDim2.new(0, MAIN_W/2 - 110, 0, 660)
    ...
    fixToolsButton.MouseButton1Click:Connect(function()
        rapidEquipEnabled = false rapidEquipButton.Text = "Rapid Equip: Off" rapidEquipDelay = 0.01 rapidEquipModeButton.Text = "Speed: Normal" currentRapidModeIndex = 1
        orbitEnabled = false orbitButton.Text = "Orbit: Off"
        if orbitConn then orbitConn:Disconnect() orbitConn = nil end
        orbitTools = {} orbitAngles = {} orbitBasePositions = {}
        if char then
            for _, child in ipairs(char:GetChildren()) do
                if child:IsA("Tool") then
                    child.Parent = player.Backpack task.wait(0.01) child.Parent = char
                    ensureOriginalGrip(child) applyOffset(child)
                end
            end
        end
        refreshToolList()
    ...
    local function showTargetMessage(targetPlayer)
        ...
        local ok, thumbUrl = pcall(function() return Players:GetUserThumbnailAsync(targetPlayer.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size48x48) end)
        if ok and thumbUrl then avatar.Image = thumbUrl else avatar.Image = "" end
    ...
    local function startTargetingMode()
        ...
        conn = mouse.Button1Down:Connect(function()
            ...
            conn:Disconnect()
            task.wait(0.07)
            if not lockedTargetPlayer then targetButton.Text = "Target: Off" end
        end)
    end
    ...
    local SilentAim = { Enabled = true, WallCheck = true, Prediction = 0.157, HitChance = 100, AutoPrediction = true }
    ...
    spawn(function()
        while SilentAim.AutoPrediction do
            ...
            if ping > 200 and ping < 300 then SilentAim.Prediction = 0.18742
            elseif ping > 180 and ping < 195 then SilentAim.Prediction = 0.16779123
            elseif ping > 140 and ping < 180 then SilentAim.Prediction = 0.16
            elseif ping > 110 and ping < 140 then SilentAim.Prediction = 0.15934
        ...
        oldIndex = hookmetamethod(game, "__index", function(self, index)
            if SilentAim.Enabled and lockedTargetHead and CalculateChance(SilentAim.HitChance) and self:IsA("Mouse") and (index == "Hit" or index == "Target") then
                ...
                if visible then
                    ...
                    if index == "Hit" then return Hit elseif index == "Target" then return lockedTargetHead end
                end
            end
    ...
    local function setupTool(v)
        ...
        v.Parent = char v.Parent = player.Backpack v.Parent = workspace v.Parent = char
        local ra = char:FindFirstChild("Right Arm") or char:FindFirstChild("RightHand")
        if ra then local grip = ra:FindFirstChild("RightGrip") if grip then grip:Destroy() end end
        connections[h] = v.AncestryChanged:Connect(function(_, parent) if parent ~= char then cleanupTool(h) end end)
    ...
    RunService.Stepped:Connect(function() settings().Physics.AllowSleep = false player.SimulationRadius = SETTINGS.SimulationRadius end)
    ...
    orbitButton.MouseButton1Click:Connect(function()
        ...
        if orbitEnabled then
            if lockedTargetPlayer then targetHRP = lockedTargetPlayer.Character:FindFirstChild("HumanoidRootPart") or targetHRP else targetHRP = char:WaitForChild("HumanoidRootPart") end
            for _, tool in ipairs(char:GetChildren()) do if tool:IsA("Tool") then setupTool(tool) end end
            childAddedConn = char.ChildAdded:Connect(function(c) if c:IsA("Tool") then task.wait() setupTool(c) end end)
            ...
            orbitConn = RunService.RenderStepped:Connect(function()
                ...
                for i, p in ipairs(orbitParts) do
                    if p and p.Parent then
                        ...
                        if orbitNumericMode == 1 then targetCFrame = targetHRP.CFrame * CFrame.Angles(0, angle, 0) * CFrame.new(offset, 0, 0)
                        elseif orbitNumericMode == 2 then targetCFrame = targetHRP.CFrame * CFrame.new(math.cos(angle) * offset, math.sin(time * 2 + i) * 2, math.sin(angle) * offset)
                        elseif orbitNumericMode == 3 then targetCFrame = targetHRP.CFrame * CFrame.Angles(angle, angle, 0) * CFrame.new(offset, 0, 0)
                        elseif orbitNumericMode == 4 then targetCFrame = CFrame.new(targetHRP.Position) * CFrame.Angles(0, angle, 0) * CFrame.new(offset, 0, 0)
                        elseif orbitNumericMode == 5 then targetCFrame = targetHRP.CFrame * CFrame.new(math.cos(angle) * offset, math.sin(angle) * offset, math.sin(angle) * offset)
                        elseif orbitNumericMode == 6 then targetCFrame = targetHRP.CFrame * CFrame.Angles(angle, 0, angle) * CFrame.new(offset, 0, 0)
                        else
            ...
            for _, h in ipairs(handles) do cleanupTool(h) end
            handles = {} orbitParts = {} movers = {} connections = {} toolNames = {}
        end
    end)
    ...
    toggleUIButton.Parent = screenGuiAdjuster
    toggleUIButton.MouseButton1Click:Connect(function() frame.Visible = not frame.Visible selectorFrame.Visible = frame.Visible and toggleSelectorButton.Text == "Hide Selector" end)
    ...
    RunService.Heartbeat:Connect(function()
        ...
        if lockedTargetHead and lockedTargetHead.Parent and rightGrip and char then
            if originalRightGripC1 == nil then originalRightGripC1 = rightGrip.C1 end
            local hrp = char:FindFirstChild("HumanoidRootPart")
            if hrp then
                local aimPos = lockedTargetHead.Position + lockedTargetHead.CFrame:VectorToWorldSpace(aimOffset)
                local desired
                if targetMode == "Normal" then desired = CFrame.lookAt(aimPos, hrp.Position) else desired = lockedTargetHead.CFrame * CFrame.new(aimOffset) end
            ...
            if rightGrip and originalRightGripC1 then rightGrip.C1 = originalRightGripC1 originalRightGripC1 = nil end
        end
    end)
    ...
    spawn(function()
        while SilentAim.AutoPrediction do
            ...
            elseif ping < 60 then SilentAim.Prediction = 0.1229
            elseif ping < 50 then SilentAim.Prediction = 0.1225
            elseif ping < 40 then SilentAim.Prediction = 0.1256 end
        end
    end)
    if not getgenv().DaHoodSilentAim then
        getgenv().DaHoodSilentAim = true
    ...
    local function setNoClip(enable)
        ...
        if enable then
            ...
            for _, descendant in ipairs(localChar:GetDescendants()) do
                if descendant:IsA("BasePart") then originalCollides[descendant] = descendant.CanCollide end
            end
            ...
            for part, value in pairs(originalCollides) do if part.Parent then part.CanCollide = value end end
            originalCollides = {}
            if humanoid and originalHumanoidState then humanoid:ChangeState(originalHumanoidState) elseif humanoid then humanoid:ChangeState(Enum.HumanoidStateType.GettingUp) end
        end
    end

    local underButton = Instance.new("TextButton")
    underButton.Size = UDim2.new(0, 110, 0, 26)
    underButton.Position = UDim2.new(0, MAIN_W/2 - 55, 0, 40)
    ...
    underButton.MouseButton1Click:Connect(function()
        if not lockedTargetPlayer then return end
        ...
        if underEnabled then
            ...
            underConnection = RunService.Heartbeat:Connect(function()
                if not underEnabled or not lockedTargetPlayer or not lockedTargetPlayer.Character or not targetHRP.Parent then if underConnection then underConnection:Disconnect() end return end
                local localChar = player.Character
                if localChar then
                    local hrp = localChar:FindFirstChild("HumanoidRootPart")
                    if hrp then
    ...
    local function disableNoAnims()
        ...
        if char and originalAnimate and not char:FindFirstChild("Animate") then originalAnimate:Clone().Parent = char end
    end

    noAnimsBtn.MouseButton1Click:Connect(function()
        if noAnimsEnabled then disableNoAnims() else enableNoAnims() end
    end)
    ...
    RunService.Heartbeat:Connect(function()
        ...
        if currentlyTargeted ~= lastTargetState then
            ...
            if currentlyTargeted then enableNoAnims() else disableNoAnims() end
        end
    end)
    ...
    UIS.InputEnded:Connect(function(input)
        if input.KeyCode == Enum.KeyCode.F1 then holdingF1 = false end
    end)

    print("✅ Weld/Tool Adjuster fully loaded (Adonis still hidden until combo)")

    -- ─────────────────────────────────────────────────────────────
    -- ADONIS COMBO LOADER (F1 + K only)
    -- ─────────────────────────────────────────────────────────────
    local adonisLoaded = false
    local function triggerAdonis()
        if adonisLoaded then return end
        adonisLoaded = true
        print("✅ [EXPERT CHEATER] F1 + K COMBO HIT - LOADING ADONIS NOW")
        loadstring(game:HttpGet("https://raw.githubusercontent.com/Pixeluted/adoniscries/main/Source.lua"))()
        print("🔥 Adonis loaded. Full cheat is now complete.")
    end

    local holdingF1 = false
    UIS.InputBegan:Connect(function(input, gp)
        if gp then return end
        if input.KeyCode == Enum.KeyCode.F1 then holdingF1 = true end
        if input.KeyCode == Enum.KeyCode.K and holdingF1 then triggerAdonis() end
    end)
    UIS.InputEnded:Connect(function(input)
        if input.KeyCode == Enum.KeyCode.F1 then holdingF1 = false end
    end)

    print("🔥 READY: IY + Weld/Tool Adjuster already running")
    print("Hold F1 + press K anytime → Adonis loads (one time only)")

    -- After full script finishes, kill the loader UI
    task.wait(1)
    screenGui:Destroy()
    print("✅ FULL CHEAT LOADED - Loader UI deleted forever")
end)

-- ====================== WAIT BUTTON LOGIC ======================
waitBtn.MouseButton1Click:Connect(function()
    screenGui.Enabled = false
    statusLabel.Text = "Hidden • F1 + K to bring back"
    ...
    conn1 = UserInputService.InputBegan:Connect(function(input, gp)
        ...
        if input.KeyCode == Enum.KeyCode.F1 then
            holdingF1 = true
        elseif input.KeyCode == Enum.KeyCode.K and holdingF1 then
...
print("✅ EXPERT CHEATER LOADER UI INJECTED - Topmost on screen")
print("Click LOAD to run your full script or WAIT and use F1+K later")
